...
async def doc(bot, update):
    ...
    await ms.edit("Preparing to recieve File...")
    c_time = time.time() 
    try:
       if type == "document":
          await bot.send_document(
	        update.message.chat.id,
                   ...
                   caption=caption, 
                   progress=progress_for_pyrogram,
                   progress_args=( "**Receiving file from my server**",  ms, c_time))
       elif type == "video": 
...
async def next_page(bot, query):
    ...
    btn.insert(0,
        [ 
	    InlineKeyboardButton(text="⚡HOW TO DOWNLOAD⚡", url='https://telegram.me/R_MvzZ')
        ] 
    )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data == "close_data":
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if grpid is not None:
                ...
                try:
                    ...
                    return await query.answer('LOVE FROM R_MvzZ ')
            else:
                await query.message.edit_text(
                    "I'm not connected to any groups!\nCheck /connections or connect to any groups",
                    quote=True
                )
                return await query.answer('Share Us R_MvzZ')
        ...
        return await query.answer('♥️Share Us R_MvzZ ♥️')
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                #'▶ Gen Stream / Download Link', callback_data=f'generate_stream_link:{file_id}'
                ...
                await query.answer('the file sent to you privately. Please Check PM Sirji / Madamji', show_alert=True)
        ...
        await query.answer('♥️Share Us R_MvzZ ♥️')
        ...
        # Delete callback query message
            ...
            print(e)  # print the error message
            await query.answer(f"☣something went wrong Sirji / Madamji\n\n{e}", show_alert=True)
            return
    ...
    #     )
        ...
        await query.answer("Hey Sirji / Madamji, please send me the last media with quote from your group.\nAnd also make sure that i am admin in your beloved group...")
        return
...
async def auto_filter(client, msg, spoll=False):
    if not spoll:
        ...
        if 2 < len(message.text) < 100:
            ...
            if not files:
                ...
                l = await message.reply_text(text=f"△ 𝙷𝚎𝚢 𝚜𝚘𝚗𝚊 `{message.from_user.first_name}` 😎,\n\nYour request has been successfully sent to Admin Dashboard !\nPLEASE BE PATIANCE! \n➟ 📝Title: `{search}`\n➟ 👮Requested By: `{message.from_user.first_name}`\n\nJoin:- {MAIN_CHANNEL_USRNM} ",
                                             reply_markup=InlineKeyboardMarkup([
                                                 [InlineKeyboardButton("Add me to group", url=f'http://telegram.me/MissValentinaa_Bot?startgroup=true')],
                                                 [InlineKeyboardButton("BACKUP CHANNEL", url=f"https://telegram.me/{DEV_CHANNEL_USRNM}"),
                                                  InlineKeyboardButton("UPDATES CHANNEL", url=f"https://telegram.me/{MAIN_CHANNEL_USRNM}")],
                                                 [InlineKeyboardButton("CLOSE", callback_data="close_data")]]))
    ...
    btn.insert(0,
        [ 
	    InlineKeyboardButton(text="⚡ʜᴏᴡ to ᴅᴏᴡɴʟᴏᴀᴅ⚡", url='https://telegram.me/R_MvzZ'),
        ] 
    )
    if offset != "":
