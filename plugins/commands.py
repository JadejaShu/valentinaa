...
async def start(client, message):
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        try:
            ...
            logger.error("HeyDear, Ek dfa check kr lo ki main Channel mei Add hu ya nhi...!")
            return
        btn = [
            [
             ...
             [
                InlineKeyboardButton(
                    "🦋 SUBSCRIBE YT Channel 🦋", url='https://telegram.me/REQUEST_MOvizZ'
                )
            ]
        ]
        ...
        if message.command[1] != "subscribe":
            try:
                ...
                btn.append([InlineKeyboardButton(" 🔄 Try Again", url=f"https://telegram.me/MissValentinaa_Bot?start={message.command[1]}")])
...
async def delete_all_index_confirm(bot, message):
    ...
    await message.answer('♥️Share Us R_MvzZ ♥️')
